function [params, varargout] = vonMisesFit(xdata, zdata, params0, lbCell, ubCell, varargin)
...
%      output: As in lsqcurvefit
  ...
  xRad = circ_ang2rad(xdata);
  [thetaRad, kappa] = circ_vmpar(xRad, zdata);
  thetaDeg = circ_rad2ang(thetaRad);
  disp(thetaDeg);
  disp(kappa);
  yFit = circ_vmpdf(xRad, thetaRad, kappa)
  coefficients = polyfit(zdata, yFit, 1)
